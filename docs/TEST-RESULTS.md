...
생성 시각: 2026-09-14T10:48:16.862Z
...
| 두 브라우저 WebRTC 전송 결합·원장 취소 | PASS | `pnpm test:e2e` | chromium-dtls-certificate-v1, 수락 전 0B/후 4538B, 취소 표시 6691ms | `artifacts/webrtc-run.json` |
| 반응형·주요 제품 브라우저 시험 | PASS | `pnpm test:e2e` | 11건 통과, 0건 실패, 83083ms | `artifacts/playwright-results.json` |
| 대표 화면 다중 viewport 캡처·육안 검수 | PASS | `pnpm test:e2e` | 14개 PNG, 390/768/1440 overflow와 200% keyboard 접근 PASS | `artifacts/screenshots/` |
...
| 공동 복구 UI의 실제 EIP-712 트랜잭션 | PASS | `pnpm test:e2e` | qbft-live-eip712-recovery-v1, 승인자 2명, epoch 5→6→7 (관리자·정지키 교체; 승인용 Ed25519 키는 데모에서 유지) | `artifacts/governance-run.json` |
