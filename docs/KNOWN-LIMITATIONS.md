...
- 공동 운영 화면의 서로 다른 로그인 서명 기록은 UX 증거다. 실제 EIP-712 복구는 계약·Foundry에서 검증했으나 UI/API 제출까지 연결되지 않았다.
- 개별 승인 취소는 DB 상태, 위임 취소는 원장 상태다. 현재 데모는 한 승인에 새 위임 하나를 발급해 취소 장면을 명확히 한다.
- active page 상태 polling은 3초이며 이번 E2E의 취소 요청→표시 철회는 4.411초였다. 이는 체인 확정·witness 관측·브라우저 polling을 포함한다. monotonic deadline 이후 UI timer jitter J≤1초는 반복 측정하지 않았고 백그라운드에서는 보장하지 않는다.
...
- 실제 기관·통신사 연동, 채택 의사, 효과·비용, 피싱 탐지 정확도, 공개 URL은 검증되지 않았다.
