...
import recordedReport from "../../../../artifacts/test-results.json";
...
export function EvidencePage() {
  ...
  const latest = useQuery({ queryKey: ["latest-evidence"], queryFn: () => api<LatestEvidence>("/evidence/latest"), refetchInterval: publicPreview ? false : 5_000, enabled: !publicPreview });
  const ready = !publicPreview && latest.data?.readiness.status === "ready";
  const report = publicPreview ? recordedReport : latest.data?.report;
  const rows = report?.checks.filter((check) => ["protocol-unit", "database-integration", "qbft-readiness", "webrtc-e2e", "governance-live-ui"].includes(check.id)) ?? [];
  return <div className="content-page"><div className="page-heading"><div><span className={`mode-tag ${ready ? "live" : "preview"}`}><Radio size={14} />{ready ? "LIVE · 원장과 witness 관측" : publicPreview ? "RECORDED · 저장된 로컬 실행" : "연결 확인 중"}</span><h1>검증 근거</h1><p>실시간 준비 상태와 저장된 실행 증거를 구분해 보여줍니다.</p></div>{publicPreview ? <a className="button secondary" href="https://github.com/0u-Y/CallSign/blob/main/artifacts/test-results.json"><ExternalLink size={18} />GitHub 증거 JSON</a> : <a className="button secondary" href="/api/evidence/latest" download><Download size={18} />현재 증거 JSON</a>}</div>
    <section className="evidence-summary"><article><Server /><div><span>QBFT 원장</span><strong>{publicPreview ? "4/4 기록" : latest.isLoading ? "확인 중" : `${latest.data?.readiness.nodeCount ?? 0}/4 노드`}</strong><small>{publicPreview ? "저장된 로컬 실행 결과" : latest.data?.readiness.sameBlockHash ? `동일 블록 #${latest.data.readiness.commonBlockNumber}` : "공통 블록 확인 불가"}</small></div></article><article><ShieldCheck /><div><span>상태 증언</span><strong>{publicPreview ? "4/4 기록" : latest.isLoading ? "확인 중" : `${latest.data?.readiness.witnessCount ?? 0}/4 witness`}</strong><small>수락 임계값 {publicPreview ? 3 : latest.data?.readiness.witnessThreshold ?? 3} · 네이티브 light client 아님</small></div></article><article><Database /><div><span>최근 테스트</span><strong>{report ? `PASS ${report.summary.pass}` : "결과 파일 없음"}</strong><small>{report ? new Date(report.generatedAt).toLocaleString("ko-KR") : "pnpm test:report 필요"}</small></div></article></section>
    <section className="results-table"><div className="panel-heading"><div><h2>저장된 실행 항목</h2><p>{publicPreview ? "공개 배포가 원장을 다시 실행한 결과가 아니라, 저장소의 마지막 로컬 실행 증거입니다." : "artifacts/test-results.json에 기록된 마지막 실제 실행입니다."}</p></div></div><div className="table-scroll"><table><thead><tr><th>검사</th><th>상태</th><th>관측</th><th>증거</th></tr></thead><tbody>{rows.length ? rows.map((row) => <tr key={row.id}><td>{row.label}</td><td><span className={`result ${row.status === "PASS" ? "pass" : "blocked"}`}>{row.status}</span></td><td>{row.observation}</td><td><code>{row.evidence}</code></td></tr>) : <tr><td colSpan={4}>아직 저장된 실행 보고서가 없습니다.</td></tr>}</tbody></table></div></section>
